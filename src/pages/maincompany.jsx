...
const MainCompany = () => {
  ...
  const handleScanResult = (scannedData) => {
    try {
      ...
      setResults(parsed);
      setSearchTerm(parsed.email || parsed.phone || parsed.firstName || "");
      localStorage.setItem("currentBuyer", JSON.stringify(parsed));
    } catch (e) {
  ...
  const stopScanner = () => {
    if (scannerInstance) {
      scannerInstance
        ...
        .catch((err) => {
          console.error("เกิดข้อผิดพลาดขณะปิดกล้อง:", err);
        });
    }
  };
        ...
        {results && (
          <div style={{ backgroundColor: "white", padding: "2rem", borderRadius: "20px", width: "400px", margin: "2rem auto", boxShadow: "0 0 10px rgba(0, 0, 0, 0.1)" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: "1rem" }}>
              <button style={{ backgroundColor: "#ff4d4d", color: "#fff", padding: "0.5rem 1rem", border: "none", borderRadius: "5px" }}>
                ยกเลิก
              </button>
              <button
